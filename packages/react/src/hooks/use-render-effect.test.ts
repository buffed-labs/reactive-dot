...
describe("useRenderEffect", () => {
  ...
  it("should work with different key types", () => {
    ...
    // Number
    const { rerender: rerenderNumber } = renderHook(({ key }) => useRenderEffect(effect, key), {
      initialProps: { key: 1 },
    });
    ...
    const obj1 = { test: 1 };
    const obj2 = { test: 1 };
    const { rerender: rerenderObject } = renderHook(({ key }) => useRenderEffect(effect, key), {
    ...
    // Boolean
    const { rerender: rerenderBoolean } = renderHook(({ key }) => useRenderEffect(effect, key), {
