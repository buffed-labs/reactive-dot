...
// ...
// highlight-next-line
import { polkadot, polkadot_asset_hub, polkadot_people } from "@polkadot-api/descriptors";
...
export function MyComponent() {
  ...
  return (
    <div>
      ...
      <section>
        <div>Latest block timestamp: {new Date(Number(timestamp)).toLocaleString()}</div>
