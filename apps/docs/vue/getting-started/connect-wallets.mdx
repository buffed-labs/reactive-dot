...
Wallets & accounts connection can by managed via [`useWallets`](/vue/api/vue/functions/useWallets), [`useConnectedWallets`](/vue/api/vue/functions/useConnectedWallets) & [`useAccounts`](/vue/api/vue/functions/useAccounts) composables.

:::tip
If you prefer not having to build a wallet connection UI from scratch, checkout [DOTConnect](https://dotconnect.dev/) for a quick and easy way to add a great wallet experience to your DApps.
:::
...
The wallets listed below require extra UI integration and platform-specific setup. To avoid implementing these flows yourself, we recommend using [DOTConnect](https://dotconnect.dev/) for a quick and reliable wallet integration.
    ...
    <article>
      ...
      <ul>
        <li v-for="wallet in wallets" :key="wallet.id">
          <div>{{ wallet.name }}</div>
          <div>
            <button v-if="connectedWallets.includes(wallet)" @click="disconnectWallet(wallet)">
              Disconnect
            </button>
